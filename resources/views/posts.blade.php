...
@section('content')
    ...
    @if($posts->count())
        <div class="card mb-3">
            ...
            <p>
                <small> By.
                        <a href="/authors/{{$posts[0]->author->username }}" class="text-decoration-none"> {{$posts[0]->author->username }}</a>
                        in <a href="/categories/{{ $posts[0]->category->slug }}" class="text-decoration-none mx-3">{{ $posts[0]->category->name }}</a>
                ...
                @foreach($posts->skip(1) as $post)
                    <div class="col-md-4">
                        <div class="card" >
                            ...
                            <p>
                                <small> By.
                                        <a href="/authors/{{$post->author->username }}" class="text-decoration-none mx-3"> {{$post->author->username }}</a>{{ $post->created_at->diffForHumans() }}
                                </small>
                            </p>
